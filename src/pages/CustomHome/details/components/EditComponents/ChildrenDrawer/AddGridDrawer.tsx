...
interface CarouselChildProps extends FormComponentProps {
  ...
  handleAddGridModalVisible: (flag?: boolean) => void
  handleAddButtons: (fieldsValue: ButtonsType) => void;
  Buttons: Array<any>
}
...
class CarouselChildForm extends Component<CarouselChildProps, CarouselChildState> {
  ...
  okHandle = () => {
    const { form, Buttons, handleAddButtons } = this.props;
  ...
  render() {
     const { form, AddGridModalVisible, handleAddGridModalVisible } = this.props;
     ...
     return (
        <>
          <Drawer
            ...
            width={500}
            destroyOnClose={true}
            onClose={() => handleAddGridModalVisible(false)}
            className={styles['drawer-warp']}
            afterVisibleChange={this.handleAfter}
            visible={AddGridModalVisible}
